									...
									<table class="table table-bordered table-striped">
										<thead class="">
											<tr>
												...
												<?php for($i=1;$i<=12;$i++){ ?>
												<th width="100px" class="text-center"><?= $i ?></th>
												<?php } ?>
											</tr>
										</thead>
										<tbody>
											...
											<tr>
												...
												for($i=1;$i<=12;$i++){
													...
													$btn = "danger";
													$btn_text = "cross";
													$buku = $this->m->Get_ResumeByBulan($i,$tahun,$u->username);
													foreach($buku as $b){
														$link=base_url().ucfirst($this->session->userdata('level'))."/ViewResumeBuku/".$b->id_buku;
														if($b->status==1){
															$btn = "success";
															$btn_text = "checkmark4";
														}else{
															$btn = "info";
															$btn_text = "radio-unchecked";
														}
													}
													?>
												<td class="text-center">
													<a class="btn bg-<?= $btn ?>" href="<?= $link ?>">
													 <i class="icon-<?= $btn_text ?>"></i>
													</a>
												</td>
												<?php } ?>
											</tr>
