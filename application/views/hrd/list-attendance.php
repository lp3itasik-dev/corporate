										...
										<div class="col-lg-2 col-9">
											<select name="bulan" class="form-control select-search" data-fouc>
												...
												for($i=1;$i<=12;$i++){
													...
													echo '<option '.$select.' value="'.$a.$i.'">'.$bln[$i].'</option>';
												}?>
											</select>
										</div>
										<label class="col-form-label col-1">Karyawan <span class="float-right">:</span></label>
										<div class="col-lg-2 col-9">
											<select name="username" class="form-control select-search" data-fouc>
												<option value="">Pilih Karyawan</option>
												<?php
												foreach($read_users as $u){
													$select="";
													if($u->username==$username){$select="selected";}
													echo '<option '.$select.' value="'.$u->username.'">'.$u->fullname.'</option>';
												}?>
											</select>
										</div>
										...
										<tbody>
											<?php
												...
												for($i=1;$i<=$tanggal;$i++){
													...
													if($i>9){$a="";}
													$list = $this->m->Get_AttendanceByTanggalAndUser($tahun.'-'.$bulan.'-'.$a.$i,$username);
													foreach($list as $a){
